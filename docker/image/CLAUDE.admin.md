...
- **Browser**: `playwright` with Chromium system libs (no browser binaries, see Local servers & loopback section)
...
### Local servers & loopback
Loopback (`localhost`, `127.0.0.1`, `::1`) is pre-exempted from the proxy via `no_proxy`, so local dev servers and browsers reach it directly. One thing to still handle: `localhost` resolves only to `::1` here, so bind servers to all interfaces (e.g. Vite needs `--host`) — otherwise they land on `::1` only and `127.0.0.1` is refused.
