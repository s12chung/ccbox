...
### Key components
The devbox lifecycle is driven by the **`ccbox`** Go CLI (cobra), which talks to the Docker Engine SDK in-process. Subcommands:
  - `ccbox build` — builds the image from the embedded context on BuildKit (buildx library)
  - `ccbox run` — runs the devbox container interactively behind the wall, wiring the local terminal to the container's pty
