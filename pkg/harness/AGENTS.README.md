...
`AGENTS.user.md` is shared amongst all CLIs dynamically. For example, with claude, `~/.claude/CLAUDE.md` is a symlink to `~/.ccbox/tmp/claude/CLAUDE.md`, which is mounted into the container at `ccbox run`. If any changes are made to it during the run, that changed copy will be placed in `claude/CLAUDE.md`.
