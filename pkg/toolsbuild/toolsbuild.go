...
// Build compiles the ccboxtools main package into outPath for linux/<goarch>.
// It must run from the repo root (cwd).
...
func buildCmd(ctx context.Context, goarch, outPath string) *exec.Cmd {
	...
	cmd.Dir = "ccboxtools" // relative to the repo root cwd
